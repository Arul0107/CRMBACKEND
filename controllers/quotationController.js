...
  try {
    // Populate the 'businessId' field to get access to BusinessAccount details
    // Added 'mobileNumber' and explicitly 'businessName' to populate from BusinessAccount
    const quotations = await Quotation.find()
      .populate('businessId', 'contactName email phone address gstin mobileNumber businessName') // 'businessName' explicitly included here
      .sort({ createdAt: -1 }); // Fetch all quotations, sorted by creation date descending
    res.json(quotations);
  } catch (err) {
...
    // The 'items' array, including 'productName' for each item, is part of req.body
